...
type LoadBalancerController struct {
	...
	loadBalancerStates      map[apimachinerytypes.NamespacedName]loadBalancerState
...
func NewLoadBalancerController(
	...
	c := &LoadBalancerController{
		...
		loadBalancerStates:    make(map[apimachinerytypes.NamespacedName]loadBalancerState),
		localIPDetector:       localIPDetector,
	}
	ipAssigner, err := ipassigner.NewIPAssigner(nodeIP, loadBalancerDummyDevice)
	if err != nil {
...
func (c *LoadBalancerController) getLoadBalancerState(service *corev1.Service) (loadBalancerState, bool) {
...
func (c *LoadBalancerController) saveLoadBalancerState(service *corev1.Service, state loadBalancerState) {
	c.loadBalancerStatesMutex.Lock()
	defer c.loadBalancerStatesMutex.Unlock()
	name := apimachinerytypes.NamespacedName{
		Namespace: service.Namespace,
...
func (c *LoadBalancerController) syncService(key apimachinerytypes.NamespacedName) error {
	...
	state, exist := c.getLoadBalancerState(service)
	...
	if selectNode {
		...
		c.saveLoadBalancerState(service, state)
	}
