...
func (c *ServiceExternalIPController) syncService(key apimachinerytypes.NamespacedName) error {
	...
	if selectNode {
		nodeName, err := c.cluster.SelectNodeForIP(currentExternalIP, ipPoool, filters...)
		if err != nil {
			return err
		}
		klog.InfoS("SelectNodeForIP", "nodeName", nodeName, "currentExternalIP", currentExternalIP, "ipPoool", ipPoool)
