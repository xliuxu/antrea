...
func TestAddService(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Service with empty ExternalIPPool annotation",
			externalIPPool: []*antreacrds.ExternalIPPool{
				newExternalIPPool("eip1", "", "1.2.3.4", "1.2.3.5"),
			},
			service:            newService("svc1", "ns1", corev1.ServiceTypeLoadBalancer, "", "eip1"),
...
func TestSyncService(t *testing.T) {
	...
	assert.NoError(t, err)
	checkForServiceExternalIP(t, controller, ctx, service.Name, service.Namespace, "1.2.3.4")
	...
	_, err = controller.client.CoreV1().Services(service.Namespace).Update(ctx, service, metav1.UpdateOptions{})
	assert.NoError(t, err)
	checkForServiceExternalIP(t, controller, ctx, service.Name, service.Namespace, "1.2.4.5")
	checkExternalIPPoolUsed(t, controller, ctx, "eip1", 0)
	checkExternalIPPoolUsed(t, controller, ctx, "eip2", 1)
...
func checkForServiceExternalIP(t *testing.T, controller *loadBalancerController, ctx context.Context, name, namespace, expectedExternalIP string) {
	assert.Eventually(t, func() bool {
