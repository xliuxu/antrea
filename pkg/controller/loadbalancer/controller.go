...
const (
	controllerName = "LoadBalancerController"
...
// LoadBalancerController is responsible for synchronizing the Service with type Loadbalancer
...
func NewLoadBalancerController(
	...
	c.externalIPAllocator.AddConsumer(func(ipPool string) {
...
func (c *LoadBalancerController) Run(stopCh <-chan struct{}) {
	...
	for _, svc := range svcs {
		c.updateIPAllocation(svc)
	}

	c.externalIPAllocator.ConsumerRestoreFinished()
...
func (c *LoadBalancerController) updateIPAllocation(service *corev1.Service) {
	ipPool := service.ObjectMeta.Annotations[antreaagenttypes.ServiceExternalIPPoolAnnotationKey]
	if service.Spec.Type != corev1.ServiceTypeLoadBalancer ||
		ipPool == "" || len(service.Status.LoadBalancer.Ingress) == 0 {
		return
	}
	ip := net.ParseIP(service.Status.LoadBalancer.Ingress[0].IP)
	if err := c.externalIPAllocator.UpdateIPAllocation(ip, ipPool); err != nil {
		klog.ErrorS(err, "Failed to update IP allocation", "service", service.Name, "ip", ip, "pool", ipPool)
		return
	}
	c.setIPAllocation(service)
	klog.InfoS("Allocated Service LoadBalancer IP", "service", service.Name, "ip", ip, "pool", ipPool)
}

func (c *LoadBalancerController) setIPAllocation(service *corev1.Service) {
	ipPool := service.ObjectMeta.Annotations[antreaagenttypes.ServiceExternalIPPoolAnnotationKey]
	if service.Spec.Type != corev1.ServiceTypeLoadBalancer ||
		ipPool == "" || len(service.Status.LoadBalancer.Ingress) == 0 {
		return
	...
	c.ipAllocationMap[key] = &ipAllocation{
		ip:     ip,
		ipPool: ipPool,
	}
}
...
func (c *LoadBalancerController) deleteLoadBalancer(service apimachinerytypes.NamespacedName) {
	c.ipAllocationMutex.Lock()
	defer c.ipAllocationMutex.Unlock()
	allocation, exists := c.ipAllocationMap[service]
	if exists {
		c.externalIPAllocator.ReleaseIP(allocation.ip, allocation.ipPool)
		delete(c.ipAllocationMap, service)
	}
}

func (c *LoadBalancerController) getAssignedLoadBalancerIPAllocation(service apimachinerytypes.NamespacedName) (*ipAllocation, bool) {
...
func getServiceLoadBalancerIP(service *corev1.Service) string {
	if len(service.Status.LoadBalancer.Ingress) == 0 {
		return ""
	}
...
func (c *LoadBalancerController) syncService(key apimachinerytypes.NamespacedName) error {
	...
	currentLoadBalancerIP := getServiceLoadBalancerIP(service)
	...
	if service.Spec.LoadBalancerIP != "" {
		...
		newLBIP, newIPPool, err = c.externalIPAllocator.AllocateIP()
	} else {
...
func (c *LoadBalancerController) updateService(prev, current *corev1.Service) error {
