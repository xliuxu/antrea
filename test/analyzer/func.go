...
func funcOutput(profiles []*cover.Profile) ([]fileHit, error) {
	...
	for _, profile := range profiles {
		...
		for _, f := range funcs {
			covered, _ := f.coverage(profile)
			var name string
			if f.reveiverName != "" {
				name = fmt.Sprintf("%s.%s", f.reveiverName, f.name)
			} else {
				name = f.name
			}
			hits = append(hits, fileHit{
				file: fmt.Sprintf("%s:%s", file, name),
				hit:  covered,
			})
		}
	}
